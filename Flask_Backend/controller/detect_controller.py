...
model_path = 'model/trainedModel/saved_model'
label_map_path = 'model/trainedModel/label_map.pbtxt'
output_dir = '../output'
...
def start_detection(image_path):
    ...
    if detections is not None:
        return save_detections(image_np, detections, category_index, image_path, multiple_detection)
...
def save_detections(image_np, detections, category_index, image_file, multiple_detection):
    ...
    for label, count in object_counts.items():
        temp_list.append({'type': label, 'count': count})

    output_path = os.path.join(output_dir, image_file)
    plt.imsave(output_path, image_np_with_detections)
    print(image_file)
    return jsonify({"data": temp_list, "image": image_file})
